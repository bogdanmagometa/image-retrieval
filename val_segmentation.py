...
if __name__ == "__main__":
    ...
    wandb.init(project='apparel-segmentation-val', resume='never')
    ...
    for imgs, masks in val_loader:
        ...
        all_imgs.extend([wandb.Image(img) for img in imgs.permute(0, 2, 3, 1).cpu().numpy()])
        all_masks.extend([wandb.Image(img) for img in masks.permute(0, 2, 3, 1).cpu().numpy()])
        all_pred_masks.extend([wandb.Image(img) for img in pred_masks.permute(0, 2, 3, 1).cpu().numpy()])
        
        break
    import pdb; pdb.set_trace()
    wandb.log({'pred_table': wandb.Table(["images", "GT", "pred"], list(zip(all_imgs, all_masks, all_pred_masks)))})
